...
#define WORKGROUP_SIZE1D 64 // needs to be 64 to fully use AMD GPUs
#define WORKGROUP_SIZE2D 8 // needs to be 64 to fully use AMD GPUs
#define WORKGROUP_SIZE3D 4 // needs to be 64 to fully use AMD GPUs
//#define PTX
//#define LOG
...
struct Device_Info {
	...
	inline Device_Info(const cl::Device& cl_device, const cl::Context& cl_context, const uint id) {
		...
		if(intel>=8.0f&&!uses_ram) { // fix wrong global memory capacity reporting for Intel dGPUs
#if defined(_WIN32)
...
private:
	...
	inline string enable_device_capabilities() const { return // enable FP64/FP16 capabilities if available
		...
		"\n #define def_workgroup_size "+to_string(WORKGROUP_SIZE1D)+"u"
		"\n #ifdef cl_khr_fp64"
	...
	string convertToString(const char *filename)
	{
		...
		{
			print_info("File containg the kernel code(\".cl\") not found. Please copy the required file in the folder containg the executable.");
			exit(1);
		}
		return NULL;
	}
public:
	...
	inline Device(const Device_Info& info, const char* kernelFilename) {
		const string& opencl_c_code=convertToString(kernelFilename);
...
private:
	ulong N = 0ull;
public:
	localMemory(ulong pN) { N = pN; }
	inline const ulong getSize() const { return sizeof(T)*N; }
};
class Kernel {
private:
	...
	inline void check_for_errors(const int error) {
		...
		if(error==-54) print_error("Workgrop size "+to_string(WORKGROUP_SIZE1D)+" for OpenCL kernel \""+name+"(...)\" is invalid!");
	...
	template<typename T> inline void link_parameter(const uint position, const localMemory<T>& memory) {
		check_for_errors(cl_kernel.setArg(position, memory.getSize(), NULL));
	}
...
public:
	...
	template<class... T> inline Kernel(const Device& device, const ulong N1, const ulong N2, const string& name, const T&... parameters) { // accepts Memory<T> objects and fundamental data type constants
		if(!device.is_initialized()) print_error("No OpenCL Device selected. Call Device constructor.");
		this->name = name;
		cl_kernel = cl::Kernel(device.get_cl_program(), name.c_str());
		link_parameters(0u, parameters...); // expand variadic template to link kernel parameters
		set_ranges(N1, N2, true);
		cl_queue = device.get_cl_queue();
	}
	template<class... T> inline Kernel(const Device& device, const ulong N1, const ulong N2, const ulong N3, const string& name, const T&... parameters) { // accepts Memory<T> objects and fundamental data type constants
		if(!device.is_initialized()) print_error("No OpenCL Device selected. Call Device constructor.");
		this->name = name;
		cl_kernel = cl::Kernel(device.get_cl_program(), name.c_str());
		link_parameters(0u, parameters...); // expand variadic template to link kernel parameters
		set_ranges(N1, N2, N3, true);
		cl_queue = device.get_cl_queue();
	}
	template<class... T> inline Kernel(const Device& device, const ulong N, bool dummy, const uint workgroup_size, const string& name, const T&... parameters) { // accepts Memory<T> objects and fundamental data type constants
		if(!device.is_initialized()) print_error("No OpenCL Device selected. Call Device constructor.");
		cl_kernel = cl::Kernel(device.get_cl_program(), name.c_str());
		link_parameters(0u, parameters...); // expand variadic template to link kernel parameters
		set_ranges(N, dummy, (ulong)workgroup_size);
		cl_queue = device.get_cl_queue();
	}
	template<class... T> inline Kernel(const Device& device, const ulong N1, const ulong N2, bool dummy, const uint workgroup_size, const string& name, const T&... parameters) { // accepts Memory<T> objects and fundamental data type constants
		if(!device.is_initialized()) print_error("No OpenCL Device selected. Call Device constructor.");
		cl_kernel = cl::Kernel(device.get_cl_program(), name.c_str());
		link_parameters(0u, parameters...); // expand variadic template to link kernel parameters
		set_ranges(N1, N2, dummy, (ulong)workgroup_size);
		cl_queue = device.get_cl_queue();
	}
	template<class... T> inline Kernel(const Device& device, const ulong N1, const ulong N2, bool dummy, const uint workgroup_size1, const uint workgroup_size2, const string& name, const T&... parameters) { // accepts Memory<T> objects and fundamental data type constants
		if(!device.is_initialized()) print_error("No OpenCL Device selected. Call Device constructor.");
		cl_kernel = cl::Kernel(device.get_cl_program(), name.c_str());
		link_parameters(0u, parameters...); // expand variadic template to link kernel parameters
		set_ranges(N1, N2, dummy, (ulong)workgroup_size1, (ulong)workgroup_size2);
		cl_queue = device.get_cl_queue();
	}
	template<class... T> inline Kernel(const Device& device, const ulong N1, const ulong N2, const ulong N3, bool dummy, const uint workgroup_size, const string& name, const T&... parameters) { // accepts Memory<T> objects and fundamental data type constants
		if(!device.is_initialized()) print_error("No OpenCL Device selected. Call Device constructor.");
		cl_kernel = cl::Kernel(device.get_cl_program(), name.c_str());
		link_parameters(0u, parameters...); // expand variadic template to link kernel parameters
		set_ranges(N1, N2, N3, dummy, (ulong)workgroup_size);
		cl_queue = device.get_cl_queue();
	}
	template<class... T> inline Kernel(const Device& device, const ulong N1, const ulong N2, const ulong N3, bool dummy, const uint workgroup_size1, const uint workgroup_size2, const uint workgroup_size3, const string& name, const T&... parameters) { // accepts Memory<T> objects and fundamental data type constants
		if(!device.is_initialized()) print_error("No OpenCL Device selected. Call Device constructor.");
		cl_kernel = cl::Kernel(device.get_cl_program(), name.c_str());
		link_parameters(0u, parameters...); // expand variadic template to link kernel parameters
		set_ranges(N1, N2, N3, dummy, (ulong)workgroup_size1, (ulong)workgroup_size2, (ulong)workgroup_size3);
		cl_queue = device.get_cl_queue();
	}
	inline Kernel() {} // default constructor
	inline Kernel& set_ranges(const ulong N, bool dummy, const ulong workgroup_size=(ulong)WORKGROUP_SIZE1D) {
		this->N = N;
	...
	inline Kernel& set_ranges(const ulong N1, const ulong N2, bool dummy, const ulong workgroup_size=(ulong)WORKGROUP_SIZE2D) {
		this->N = N1*N2;
		cl_range_global = cl::NDRange(((N1+workgroup_size-1ull)/workgroup_size)*workgroup_size, ((N2+workgroup_size-1ull)/workgroup_size)*workgroup_size); // make global range a multiple of local range
		cl_range_local = cl::NDRange(workgroup_size, workgroup_size);
		return *this;
	}
	inline Kernel& set_ranges(const ulong N1, const ulong N2, bool dummy, const ulong workgroup_size1, const ulong workgroup_size2) {
		this->N = N1*N2;
		cl_range_global = cl::NDRange(((N1+workgroup_size1-1ull)/workgroup_size1)*workgroup_size1, ((N2+workgroup_size2-1ull)/workgroup_size2)*workgroup_size2); // make global range a multiple of local range
		cl_range_local = cl::NDRange(workgroup_size1, workgroup_size2);
		return *this;
	}
	inline Kernel& set_ranges(const ulong N1, const ulong N2, const ulong N3, bool dummy, const ulong workgroup_size=(ulong)WORKGROUP_SIZE3D) {
		this->N = N1*N2*N3;
		cl_range_global = cl::NDRange(((N1+workgroup_size-1ull)/workgroup_size)*workgroup_size, ((N2+workgroup_size-1ull)/workgroup_size)*workgroup_size, ((N3+workgroup_size-1ull)/workgroup_size)*workgroup_size); // make global range a multiple of local range
		cl_range_local = cl::NDRange(workgroup_size, workgroup_size, workgroup_size);
		return *this;
	}
	inline Kernel& set_ranges(const ulong N1, const ulong N2, const ulong N3, bool dummy, const ulong workgroup_size1, const ulong workgroup_size2, const ulong workgroup_size3) {
		this->N = N1*N2*N3;
		cl_range_global = cl::NDRange(((N1+workgroup_size1-1ull)/workgroup_size1)*workgroup_size1, ((N2+workgroup_size2-1ull)/workgroup_size2)*workgroup_size2, ((N3+workgroup_size3-1ull)/workgroup_size3)*workgroup_size3); // make global range a multiple of local range
		cl_range_local = cl::NDRange(workgroup_size1, workgroup_size2, workgroup_size3);
		return *this;
	}
