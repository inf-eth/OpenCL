...
COpenCLTemplate::COpenCLTemplate(unsigned int pWidth, PRECISION pMultiplier): Width(pWidth), Multiplier(pMultiplier), input(NULL), output(NULL), tStart(high_resolution_clock::now()), tEnd(high_resolution_clock::now()), tDelta(0), tPaused(true)
{
...
int COpenCLTemplate::InitialiseCL()
{
	...
	*/
	...
	{
		cout << "No platforms detected. Exiting..." << endl;
		return 1;
	}
	...
	cin >> choice;
	StartTimer();
	int intChoice = ((int)choice)-(int)48;
	if (intChoice < 0 || intChoice > (int)numPlatforms)
	{
		cout << "Invalid choice, reverting to default (0)." << endl;
		intChoice = 0;
	}
	platform = platforms[intChoice];
	delete[] platforms;
	...
	*/
	...
	SafeCall(clGetDeviceIDs(platform, CL_DEVICE_TYPE_ALL, 0, NULL, &deviceCount), "clGetDeviceIDs failed");
	...
	// Get device ids
	SafeCall(clGetDeviceIDs(platform, CL_DEVICE_TYPE_ALL, deviceCount, deviceIds, NULL), "clGetDeviceIDs failed");
	...
	{
		char pbuff[100];
		SafeCall(clGetPlatformInfo(platform, CL_PLATFORM_VENDOR, sizeof(pbuff), pbuff, NULL), "Error: Getting Platform Info.(clGetPlatformInfo)");
		if(!strcmp("NVIDIA Corporation", pbuff))
...
int COpenCLTemplate::RunCLKernels()
{
	...
	// Query device capabilities. Maximum work item dimensions and the maximum work item sizes
...
string COpenCLTemplate::convertToString(const char *filename)
{
	...
	{
		cout << "\nFile containing the kernel code(\".cl\") not found. Please copy the required file in the folder containing the executable." << endl;
		exit(1);
	}
	return NULL;
}
...
void COpenCLTemplate::StopTimer()
{
	if (tPaused == false)
	{
		...
		duration<PRECISION, std::milli> ms_doubleC = tEnd-tStart;
		tDelta += ms_doubleC.count();
		tStart = tEnd;
		tPaused = true;
	}
}
...
PRECISION COpenCLTemplate::GetElapsedTime()
{
	...
	duration<PRECISION, std::milli> ms_doubleC = tEnd-tStart;
	return (ms_doubleC.count()+tDelta)/1000;
}
