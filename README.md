...
## Note for this Simplified Wrapper
The original wrapper was modified to allow multi-dimentional kernels with a cmake build system. The kernel code compilation was reverted to use the .cl files and not the hoop-jumping .cpp version that came with the original wrapper. Although this can stll be done by slight modifications.
